...
| `AGENT_DEPLOYER_RUNTIME_EXPOSE` | 否 | `public` | runtime 容器网络拓扑：`public` / `loopback` / `docker-network` / `private`。纯服务端配置——决定 runtime 容器的可达方式，以及响应是否携带服务端生成的 `upstream` 定位信息。 |
| `AGENT_DEPLOYER_RUNTIME_BIND_IP` | 仅 private 模式 | — | runtime 端口发布到的宿主 IP。仅在 `AGENT_DEPLOYER_RUNTIME_EXPOSE=private` 下必填（且只在该模式下有效）；必须是具体的可路由 IP（拒绝 loopback 与通配地址）。 |
...
| `AGENT_DEPLOYER_UPSTREAM_HOST` | 否 | — | 覆盖返回的 `upstream` 定位信息中的 host。仅在 `loopback` / `private` 模式下有效（如 `host.docker.internal`）。 |
