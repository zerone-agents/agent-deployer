...
为提高可读性,system prompt 默认外挂存储:每个 agent 的 `systemPrompt` 写入 `agents/<name>/agents/prompts/<id>-<sha256>.md`(与 `agents.yaml` 同目录、同一 bind mount),YAML entry 通过 `systemPromptFile` 引用(相对 config 目录,runtime v2.5.0)——长 prompt 不再平铺在 `agents.yaml` 中。读回时解析文件恢复为响应中的 `systemPrompt` 文本,API 形态不变。
